    ...
    public void serialize(RuleGroup<?> value, JsonGenerator gen, SerializerProvider provider) {
        try {
            ...
            value.getConditions().forEach(condition -> {
                if (condition == null) {
                    throw new RuleGroupSerializationException("Condition in RuleGroup cannot be null");
                }
                switch (condition) {
                    case Rule rule -> {
                        try {
                            ...
                            throw new RuleGroupSerializationException("Exception serializing Rule in RuleGroup", e);
                        }
                    }
                    case RuleGroup ruleGroup -> {
                        try {
                            ...
                            throw new RuleGroupSerializationException("Exception serializing nested RuleGroup", e);
                        }
                    }
                    default ->
                            throw new RuleGroupSerializationException("Unknown condition type in RuleGroup: " + condition.getClass());
                }
            });
            gen.writeEndArray();
            gen.writeEndObject();
        } catch (Exception e) {
            throw new RuleGroupSerializationException("Exception encountered while serializing RuleGroup", e);
        }
    }
}
