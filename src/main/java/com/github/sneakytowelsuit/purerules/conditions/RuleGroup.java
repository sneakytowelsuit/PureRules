...
/**
 * Represents a group of conditions (rules or nested rule groups) that are evaluated together
 * using a specified combinator (AND/OR), with optional inversion and bias. Each group is uniquely identified.
 * Default values are set to ensure safe evaluation, but can be overridden via the builder.
 *
...
public final class RuleGroup<TInput> implements Condition<TInput> {
    ...
    @Builder.Default
    private final List<Condition<TInput>> conditions = new ArrayList<>();
    @Builder.Default
    private final Combinator combinator = Combinator.AND;
    @Builder.Default
    private final boolean isInverted = false;
    /**
     * Bias to use when the group contains no conditions.
     * Defaults to EXCLUSIVE (pessimistic).
     */
    @Builder.Default
    private final Bias bias = Bias.EXCLUSIVE;

    /**
     * Evaluates this rule group against the given input.
     * Uses the current thread ID and the group's ID as the evaluation context.
     *
    ...
    public boolean evaluate(TInput input) {
        ...
        return evaluateConditions(input, ConditionUtils.getIdPath(this,null), Thread.currentThread().threadId());
    }
    ...
    private boolean evaluateConditions(TInput input, List<String> parentIdPath, Long threadId) {
        ...
        boolean result = handleConditions(
    ...
    private boolean handleConditions(
        ...
        return switch (Optional.ofNullable(combinator).orElse(Combinator.AND)) {
            case AND -> simpleRules.stream().allMatch(r -> r.evaluate(input, parentIdPath, threadId))
                    && complexRules.stream().allMatch(r -> r.evaluateConditions(input, parentIdPath, threadId));
            case OR -> simpleRules.stream().anyMatch(r -> r.evaluate(input, parentIdPath, threadId))
                    || complexRules.stream().anyMatch(r -> r.evaluateConditions(input, parentIdPath, threadId));
        } ^ this.isInverted;
    }
